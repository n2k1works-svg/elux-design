...
Task ID: 5
Agent: Main Agent
Task: Fix projects carousel for 12-15 project support

Work Log:
- Identified bug: desktop projects used `grid grid-cols-3` with `translateX` — CSS Grid wraps items into rows, so sliding breaks beyond 3 projects
- Fixed by changing to `flex` layout with `gap-6`, each card gets `flex-shrink-0` and `width: calc(33.333% - 1rem)`
- All projects now render in a single flex row; `translateX(-${page * 100}%)` shifts exactly 3 cards per page
- Removed unused `desktopProjects` slice variable (all items in carousel, CSS handles visibility)
- Navigation arrows and dot indicators unchanged — still work correctly
- No hard limit on project count — 12-15 projects will create 4-5 pages of 3 cards each
- Pushed to GitHub (commit d21fcad7f1f1), Vercel will auto-deploy

Stage Summary:
- Projects carousel now properly supports 12-15+ projects with smooth sliding
- Arrow navigation + dot indicators work for any number of projects (3 per page on desktop)
...
Task ID: 1
Agent: Main Agent
Task: Fix admin dashboard not matching Elux website - public sections were hardcoded instead of reading from DB

Work Log:
- Investigated the mismatch: discovered ServicesSection and AboutSection were rendering hardcoded data, ignoring admin changes saved to DB
- Created ServiceIcon component to map iconKey strings (building, 3d, oversight) to SVG icons
- Added FALLBACK_SERVICES and FALLBACK_ABOUT constants so the site looks good even without DB data
- Rewrote ServicesSection to fetch from /api/services with fallback to FALLBACK_SERVICES
- Rewrote AboutSection to fetch from /api/about with fallback to FALLBACK_ABOUT, including dynamic stat labels and counter values
- Updated Footer to fetch settings from /api/settings for dynamic location and social links
- Removed leftover scripts/new-modal.tsx that was causing build errors
- Added 'skills' to tsconfig.json exclude to prevent build errors from skill scripts

Stage Summary:
- Build passes cleanly
- All 3 sections (Services, About, Footer) now dynamically read from the same DB the admin writes to
- If no DB data exists, fallback defaults are used (matching original hardcoded content)
- Process section and Why Elux section remain hardcoded (no admin editor exists for them)
