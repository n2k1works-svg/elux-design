  ...
  return (
    <>
      ...
      {lightboxImage && (
        ...
        >
          <button className="absolute top-6 right-6 w-10 h-10 rounded-full liquid-glass-subtle flex items-center justify-center text-[#C9A84C] hover:bg-[rgba(201,168,76,0.15)] transition-all z-10" onClick={() => { setLightboxImage(null); setLightboxImages([]); }} aria-label="Close image viewer">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
          {lightboxImages.length > 1 && (
            <button className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full liquid-glass-subtle flex items-center justify-center text-[#C9A84C] hover:bg-[rgba(201,168,76,0.15)] transition-all z-10" onClick={(e) => { e.stopPropagation(); const ni = (lightboxIndex - 1 + lightboxImages.length) % lightboxImages.length; setLightboxIndex(ni); setLightboxImage(lightboxImages[ni]); }} aria-label="Previous image">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
            </button>
          )}
          {lightboxImages.length > 1 && (
            <button className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full liquid-glass-subtle flex items-center justify-center text-[#C9A84C] hover:bg-[rgba(201,168,76,0.15)] transition-all z-10" onClick={(e) => { e.stopPropagation(); const ni = (lightboxIndex + 1) % lightboxImages.length; setLightboxIndex(ni); setLightboxImage(lightboxImages[ni]); }} aria-label="Next image">
          ...
          {lightboxImages.length > 1 && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
              {lightboxImages.map((_, i) => (
                <button key={i} className={`w-2 h-2 rounded-full transition-all ${i === lightboxIndex ? 'bg-[#C9A84C] w-6' : 'bg-white/40 hover:bg-white/60'}`} onClick={(e) => { e.stopPropagation(); setLightboxIndex(i); setLightboxImage(lightboxImages[i]); }} aria-label={`Go to image ${i + 1}`} />
              ))}
            </div>
          )}
        </div>
  ...
  return (
    <>
      <div
        ...
        onClick={(e) => { e.stopPropagation(); onImageClick(currentImg); }}
      >
        {currentImg?.startsWith('data:') ? (
          <img src={currentImg} alt={project.title} className="card-image object-cover absolute inset-0 w-full h-full" />
        ) : (
          <Image src={currentImg || '/project-1.png'} alt={project.title} fill className="card-image object-cover" sizes="(max-width: 768px) 80vw, 33vw" />
        )}
        ...
        {allImages.length > 1 && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10" onClick={(e) => e.stopPropagation()}>
            {allImages.map((img, i) => (
              <button key={i} className={`w-1.5 h-1.5 rounded-full transition-all ${i === cardImgIndex ? 'bg-[#C9A84C] w-4' : 'bg-white/40 hover:bg-white/60'}`} onClick={() => setCardImgIndex(i)} aria-label={`View image ${i + 1}`} />
            ))}
          </div>
        )}
      </div>
      <div className="p-6">
        ...
        <p className="text-[#8A8478] text-xs mt-3 font-light">
          ...
          {allImages.length > 1 && <span className="ml-2 text-[#C9A84C]/60">{allImages.length} images</span>}
        </p>
      </div>
    </>
  );
    ...
    <footer className="border-t border-[rgba(201,168,76,0.1)]">
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-col items-center gap-4">
          <p className="text-center text-xs text-[#8A8478]/60 font-light">{settings.location}</p>
          <div className="flex items-center gap-4">
            {settings.facebook && (
              <SocialLink href={settings.facebook} label="Facebook"><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg></SocialLink>
            )}
            {settings.instagram && (
              <SocialLink href={settings.instagram} label="Instagram"><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" /></svg></SocialLink>
            )}
            {settings.linkedin && (
              <SocialLink href={settings.linkedin} label="LinkedIn"><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg></SocialLink>
            )}
          </div>
          <a href="https://n2klabs.vercel.app/" target="_blank" rel="noopener noreferrer" className="text-center text-xs text-[#C9A84C]/70 font-light hover:text-[#C9A84C] transition-colors duration-300">Powered by <span className="font-semibold tracking-wide">N2K Labs</span></a>
        </div>
      </div>
    </footer>
  );
          ...
          {projects.map((p) => (
            <div key={p.id} className="liquid-glass rounded-xl p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {p.images?.length > 0 && <p className="text-[0.6rem] text-[#C9A84C]/50 mt-1">{p.images.length} image{p.images.length > 1 ? "s" : ""}</p>}
...
function ProjectFormModal({ project, onClose, onSaved, onError }: {
  ...
  const uploadFiles = async (files: FileList | File[]) => {
    ...
    const validFiles = fileArr.filter(f => {
      ...
      if (f.size > MAX_FILE_SIZE) { onError(f.name + " is too large (" + (f.size/1024/1024).toFixed(1) + "MB). Max 15MB."); return false; }
      return true;
    });
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!title || !location || !category || !description) { onError("All fields are required."); return; }
    setSaving(true);
    try {
      const fd = new FormData();
      ...
      if (!r.ok) { const data = await r.json().catch(() => ({})); throw new Error(data.error || "Save failed"); }
      onSaved();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[220] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className="liquid-glass-strong rounded-2xl w-full max-w-2xl my-8 max-h-[90vh] overflow-y-auto admin-scroll" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 z-10 bg-[#0A0A0A]/80 backdrop-blur-md px-6 py-4 border-b border-[rgba(201,168,76,0.1)] flex items-center justify-between">
          <h3 className="font-display text-xl font-light text-[#F5F0E8]">{isEdit ? "Edit Project" : "New Project"}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="grid sm:grid-cols-2 gap-4">
            <AdminField label="Title"><input className={adminInputCls} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Fantasy Island Villa" required /></AdminField>
            <AdminField label="Location"><input className={adminInputCls} value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Fantasy Island, Fiji" required /></AdminField>
          </div>
          <AdminField label="Category"><input className={adminInputCls} value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Luxury Waterfront Residence" required /></AdminField>
          <AdminField label="Description"><textarea className={adminInputCls + " resize-none"} rows={4} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Project description..." required /></AdminField>

          {/* Images: upload multiple */}
          <AdminField label="Project Images" hint="Upload multiple images (max 15MB each). First uploaded becomes primary. Drag & drop or click.">
            <div
              onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
              onDragLeave={() => setDragOver(false)}
              onDrop={handleDrop}
              onClick={() => fileInputRef.current?.click()}
              className={["relative border-2 border-dashed rounded-xl px-4 py-6 text-center cursor-pointer transition-all", dragOver ? "border-[#C9A84C] bg-[rgba(201,168,76,0.08)]" : "border-[rgba(201,168,76,0.2)] hover:border-[rgba(201,168,76,0.4)]"].join(" ")}
            >
              <div className="py-2">
              ...
              <input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/jpg,image/webp,image/gif,image/svg+xml" multiple onChange={handleFileInput} className="hidden" />
            </div>
            ...
            {images.length > 0 && (
              <div className="mt-3 space-y-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {images.map((img, i) => (
                    <div key={i} className={["relative group rounded-lg overflow-hidden aspect-square", img === image ? "ring-2 ring-[#C9A84C]" : ""].join(" ")}>
                      <img src={img} alt={"Image " + (i+1)} className="w-full h-full object-cover" />
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100">
                        <button type="button" onClick={(e) => { e.stopPropagation(); setPrimaryImage(i); }} className={["w-7 h-7 rounded-full flex items-center justify-center transition-all", img === image ? "text-[#C9A84C]" : "text-white/70 hover:text-[#C9A84C]"].join(" ")} aria-label="Set as primary">
            ...
            <div className="mt-2 flex gap-2">
              <input className={adminInputCls + " flex-1 text-xs"} placeholder="Or paste primary image URL here..." value={(image.startsWith("http") || image.startsWith("/")) ? image : ""} onChange={(e) => setImage(e.target.value)} />
              <button type="button" onClick={() => setImage("/project-1.png")} className="text-[0.65rem] px-3 py-2 rounded-lg border border-[rgba(201,168,76,0.2)] text-[#8A8478] hover:text-[#C9A84C] hover:border-[rgba(201,168,76,0.4)] transition-all whitespace-nowrap">Reset</button>
            </div>
          </AdminField>

          <div className="grid sm:grid-cols-2 gap-4">
            <AdminField label="Display Order" hint="Lower numbers appear first."><input type="number" className={adminInputCls} value={order} onChange={(e) => setOrder(parseInt(e.target.value, 10) || 0)} min={0} /></AdminField>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-[rgba(201,168,76,0.1)]">
            ...
            <AdminButton type="submit" disabled={saving || uploading}>{saving ? "Saving..." : isEdit ? "Save Changes" : "Create Project"}</AdminButton>
          </div>
        </form>
      </div>
    </div>
